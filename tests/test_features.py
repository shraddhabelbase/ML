...
class TestFeatures(TestCase):
    ...
    def test_standard_scaler_transform(self):
        ...
        scaler.fit(data)
        result = scaler.transform(data)  # This line was missing
        assert (result == expected).all(), f"Scaler transform does not return expected values. Expect {expected.reshape(1,-1)}, Got: {result.reshape(1,-1)}"
    ...
    def test_standard_scaler_zero_std(self):
        scaler = StandardScaler()
        data = [[2, 2], [2, 2], [2, 2], [2, 2]]  # All values are the same, std should be 0
        scaler.fit(data)
        result = scaler.transform(data)
        expected = np.zeros_like(data)  # Transformation should result in an array of zeros
        assert (result == expected).all(), f"Scaler transform with zero std does not return expected values of zeros. Got: {result}"
